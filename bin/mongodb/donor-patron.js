...
function donationToPatron(donation) {
  ...
  if (donation.userId)
    charge.userId = donation.userId;
  if (donation.payPalTnx)
    charge.payPal.tnxId = donation.payPalTnx;
  if (donation.payPalSub)
    charge.payPal.subId = donation.payPalSub;
  if (donation.name)
    charge.payPal.name = donation.name;
  if (donation.email)
    charge.payPal.email = donation.email;
  db.plan_charge.insert(charge);

  if (userIds.indexOf(donation.userId) !== -1)
    return;
  ...
  if (donation.payPalSub)
    patron.payPal.subId = donation.payPalSub;
  if (donation.email)
    patron.payPal.email = donation.email;
