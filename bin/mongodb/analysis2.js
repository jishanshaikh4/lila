...
function decodeScore(pp) { return pp == '_' ? null : parseInt(pp); }

print('Migrating ' + max + ' analysis');

collection.drop();

let nb = 0, dat = new Date().getTime() / 1000;
gamesToMigrate.forEach(function(a) {
  ...
  try {
    ...
    for (it = 0, l = splitted.length - 1; it < l; it++) {
      ...
      let move = decodePiotr(cur[0]), best = decodePiotr(cur[1]),
          score = decodeScore(next[2]), mate = decodeScore(next[3]);
      data.push([
        score, mate == null  ? null
               : it % 2 == 1 ? mate
                             : -mate
      ].join(','));
    }
