...
function getTc(g) {
  if (g.c) {
    let h = g.c.hex();
    let lim = parseInt(h.substr(0, 2), 16);
    return [
      lim < 181 ? lim * 60 : (lim - 180) * 15, parseInt(h.substr(2, 2), 16)
    ];
  }
}
function getPerf(tc) {
...
db.user4.find(query).forEach(u => {
  ...
  let classicalPerf = u.perfs.classical;
  classicalPerf.la = undefined;
  classicalPerf.nb = NumberInt(0);
  classicalPerf.re = [];
  let rapidPerf = copyPerf(classicalPerf);
  let gameQuery = {
    us : u._id,
    s : {$gte : 30},
    ra : true,
  ...
  if (classicalPerf.nb == 0)
    update['$unset'] = {'perfs.classical' : true};
  else
    update['$set']['perfs.classical'] = classicalPerf;
  if (rapidPerf.nb > 0)
    update['$set']['perfs.rapid'] = rapidPerf;
