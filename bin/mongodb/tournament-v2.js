let orig = db.tournament;
let dest = db.tournament2;
let pairings = db.tournament_pairing;
let players = db.tournament_player;
let batchSize = 5;
let pause = 500;
...
cursor.forEach(function (o) {
  ...
  if (it % batchSize == 0) {
    ...
    let ms = Math.round(1000 * (dat2 - dat - pause / 1000));
    let perSec = Math.round(batchSize / ms);
    dat = dat2;
...
function insertPlayers(o) {
  let bulk = players.initializeUnorderedBulkOp();
...
function insertPairings(o) {
  let bulk = pairings.initializeUnorderedBulkOp();
