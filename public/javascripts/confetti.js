...
(function() {
let speed = 50, duration = 1.0 / speed, confettiRibbonCount = 11,
    ribbonPaperCount = 30, ribbonPaperDist = 8.0, ribbonPaperThick = 8.0,
    confettiPaperCount = 95, DEG_TO_RAD = PI / 180, RAD_TO_DEG = 180 / PI,
    colors = [
      [ '#df0049', '#660671' ],
      [ '#00e857', '#005291' ],
      [ '#2bebbc', '#05798a' ],
...
Vector2.Distance = function(
    _vec0, _vec1) { return sqrt(Vector2.SqrDistance(_vec0, _vec1)); };
Vector2.SqrDistance = function(_vec0, _vec1) {
  let x = _vec0.x - _vec1.x;
  let y = _vec0.y - _vec1.y;
  return x * x + y * y + z * z;
};
Vector2.Scale = function(
    _vec0, _vec1) { return new Vector2(_vec0.x * _vec1.x, _vec0.y * _vec1.y); };
...
function ConfettiPaper(_x, _y) {
  ...
  this.Draw = function(_g) {
    ...
    _g.beginPath();
    _g.moveTo((this.pos.x + this.corners[0].x * this.size) * retina,
              (this.pos.y + this.corners[0].y * this.size * this.cosA) *
                  retina);
    for (let i = 1; i < 4; i++) {
      _g.lineTo((this.pos.x + this.corners[i].x * this.size) * retina,
...
function ConfettiRibbon(_x, _y, _count, _dist, _thickness, _angle, _drag) {
  ...
  for (let i = 0; i < this.particleCount; i++) {
    this.particles[i] =
        new EulerMass(_x, _y - i * this.particleDist, this.particleDrag);
  }
  this.Update = function(_dt) {
    let i = 0;
    ...
    for (i = 1; i < this.particleCount; i++) {
      let dirP = Vector2.Sub(this.particles[i - 1].position,
    ...
    for (i = 1; i < this.particleCount; i++) {
      let rp2 = new Vector2(this.particles[i].position.x,
  ...
  this.Reset = function() {
    ...
    this.particles = [];
    for (let i = 0; i < this.particleCount; i++) {
      this.particles[i] = new EulerMass(this.position.x,
  ...
  this.Draw = function(_g) {
    for (let i = 0; i < this.particleCount - 1; i++) {
      let p0 = new Vector2(this.particles[i].position.x + this.xOff,
                           this.particles[i].position.y + this.yOff);
      let p1 = new Vector2(this.particles[i + 1].position.x + this.xOff,
      ...
      if (this.Side(this.particles[i].position.x, this.particles[i].position.y,
                    ...
                    this.particles[i + 1].position.y, p1.x, p1.y) < 0) {
        _g.fillStyle = this.frontColor;
      ...
      if (i == 0) {
        _g.beginPath();
        _g.moveTo(this.particles[i].position.x * retina,
                  this.particles[i].position.y * retina);
        _g.lineTo(this.particles[i + 1].position.x * retina,
                  this.particles[i + 1].position.y * retina);
        _g.lineTo((this.particles[i + 1].position.x + p1.x) * 0.5 * retina,
        ...
        _g.lineTo(this.particles[i + 1].position.x * retina,
                  this.particles[i + 1].position.y * retina);
        _g.lineTo((this.particles[i].position.x + p0.x) * 0.5 * retina,
  ...
  this.Side = function(
      x1, y1, x2, y2, x3,
      y3) { return (x1 - x2) * (y3 - y2) - (y1 - y2) * (x3 - x2); };
}
...
confetti.Context = function(id) {
  ...
  for (i = 0; i < confettiRibbonCount; i++) {
    confettiRibbons[i] = new ConfettiRibbon(
        random() * canvasWidth, -random() * canvasHeight * 2, ribbonPaperCount,
        ribbonPaperDist, ribbonPaperThick, 45, 0.05);
  }
  let confettiPapers = [];
  ConfettiPaper.bounds = new Vector2(canvasWidth, canvasHeight);
  for (i = 0; i < confettiPaperCount; i++) {
    confettiPapers[i] =
        new ConfettiPaper(random() * canvasWidth, random() * canvasHeight);
  }
...
window.addEventListener('resize', function(event) { confetti.resize(); });
})();
