...
let games =
    db.game4
        .find({'p.ed' : {$exists : true}},
              {'p.elo' : true, 'p.ed' : true, 'p.uid' : true, ca : true})
        .sort({ca : 1});
let dat = new Date().getTime() / 1000, it = 0;
...
function flush(cache) {
  for (uid in cache) {
    collection.update({_id : uid}, {$pushAll : {entries : cache[uid]}},
                      {upsert : true});
  }
}
...
games.forEach(function(game) {
  for (pi in game.p) {
    try {
      let p = game.p[pi], op = game.p[1 - pi];
      let entry = [
        parseInt(game.ca.getTime() / 1000), parseInt(p.elo + p.ed),
        parseInt(op.elo)
      ];
      if (typeof cache[p.uid] == 'undefined')
        cache[p.uid] = [ entry ];
      else
        cache[p.uid].push(entry);
    } catch (e) {
